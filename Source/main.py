...
def get_twits(name):
    comand = f"twint -u {name} --since 2020-01-01 --until 2020-12-31 -o ../Users/{name}.csv --csv"
    ...
    while True:
        ...
        try:
            ...
            if os.path.exists(f"{path}{name}.csv"):
                print(f"\nPlots already exist! Please, check {path}")
                break
                """
            get_twits(name)
        except ValueError:
            ...
            os.rename(f"../Users/{name}.csv", f"../Users/{name}/{name}.csv")
            cleaning(name)
            create_plots(name)
